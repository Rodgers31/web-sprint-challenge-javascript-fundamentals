...
//🚀🚀🚀 ⬇️ 📝 Explanation ⬇️ 📝 🚀🚀🚀: 
// The nested fucntion can access the variable beacuse it is within the scope of the parent function. 
...
function summation(number) {
  ...
  for(let i = 0; i <= number; i++){
    sum += i;
  // sum = sum + i is also another way to get the sum
...
const zooAnimals = [
  ...
  */
  ...
  function animalNames(animals){
    let name = [];
    animals.forEach(item => name.push(`name: ${item.animal_name}, scientific: ${item.scientific_name}`));
    return name;
  }
  ...
  */
  ...
  function lowerCaseNames(animals){
    const newArray  = animals.map(item =>item.animal_name.toLowerCase());
    return newArray;
  }
  ...
  */
  ...
  function lowPopulationAnimals(animals){
    let population = animals.filter(item => item.population<5);
    return population;
  }
  ...
  */
  ...
  function USApop(animals){
    let total = animals.reduce(function(array, item){
      return array + item.population;
    },0)
    return total;
  }
...
let cuboidTwo = new CuboidMakerTwo(2, 4, 4);
